...
const BridalPage: React.FC = () => {
  const bridalCategory = SERVICE_CATEGORIES.find(cat => cat.id === 'bridal');

  if (!bridalCategory) return null;
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative h-[60vh] bg-stone-900">
        <div
          ...
          style={{ backgroundImage: `url(${bridalCategory.services[0]?.image})` }}
        >
          <div className="absolute inset-0 bg-black/50"></div>
        </div>
        <div className="relative z-10 h-full flex flex-col justify-center items-center text-center px-4">
          <span className="text-white/90 text-xs md:text-sm tracking-[0.3em] uppercase mb-4">Services</span>
          <h1 className="text-5xl md:text-6xl font-serif text-white mb-6">{bridalCategory.title}</h1>
          <p className="text-white/90 text-lg max-w-2xl">{bridalCategory.description}</p>
        </div>
      </section>
      ...
      <section className="py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-serif text-stone-900 mb-4">Choose Your Perfect Package</h2>
            <p className="text-stone-500 font-light max-w-2xl mx-auto">
              All packages include professional makeup application and styling by our experienced team
            </p>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
            {bridalCategory.services.map((service) => (
      ...
      <section className="py-24 bg-stone-50">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="text-center mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-serif text-stone-900 mb-4">Our Bridal Work</h2>
            <p className="text-stone-500 font-light max-w-2xl mx-auto">
              See how we've helped brides look stunning on their special day
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {bridalCategory.services.map((service, idx) => (
